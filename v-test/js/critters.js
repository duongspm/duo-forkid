...
function buildCritter(spec) {
  ...
  if (spec.ears === 'pointy') {
    ...
    const earGeo = new THREE.CapsuleGeometry(0.22, 0.4, 2, 6);
    [-1, 1].forEach(s => {
  ...
  let tail;
  if (spec.tail === 'long') {
    tail = new THREE.Mesh(new THREE.CapsuleGeometry(0.09, 0.9, 2, 6), bodyMat);
    ...
    tail = new THREE.Mesh(new THREE.CapsuleGeometry(0.05, 0.5, 2, 6), bodyMat);
